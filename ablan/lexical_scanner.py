...
# import dat.vop,dat.alop,dat.keywords
        ...
        while True:
            ...
            while i <length -1:#length-1 为 行末尾\n
                ...
                if read[i] == " ":
                    ...
                    if read[i+1] == "=":
                        i +=1
                        djangoResult.append("  value giving operator --- +=")
                        lexToDjango.append({read[i-1:i+1]:vop[read[i-1:i+1]]})
                    else :
                        djangoResult.append("  algorithm and logical operators --- +")
                ...
                # elif read[i] == "&":
                #     djangoResult.append("  Speacial character --- &\n")
                #TODO:new catagory completed
                #elif read[i] == ",":
                #    djangoResult.append("  Speacial character --- ,")
                elif read[i] == "(":
                    djangoResult.append("  Speacial character --- (")#左右括号匹配交给语法分析
                    lexToDjango.append({read[i]:700})
                elif read[i] == ")":
                    djangoResult.append("  Speacial character --- )")
                    lexToDjango.append({read[i]:701})
                elif read[i] == "{":
                    djangoResult.append("  Speacial character --- {")
                    lexToDjango.append({read[i]:702})
                elif read[i] == "}":
                    djangoResult.append("  Speacial character --- }")
                    lexToDjango.append({read[i]:703})    
                elif read[i] == ":":
                    djangoResult.append("  Speacial character --- :")
                    lexToDjango.append({read[i]:704})
...
def keywordRecg(Slice):
    if Slice in keywords:
        ...
        lexToDjango.append({Slice:keywords[Slice]})
    else:
...
def test():
    getToken('D:\\Django\\compile\\ablan\\example\\syntax_correct_test.txt')
